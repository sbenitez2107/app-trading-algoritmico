...
| [MEASURED_Demo_vs_Backtest_Divergence.md](MEASURED_Demo_vs_Backtest_Divergence.md) | **`measured-data`** | 📊 **Measured, from `AppTA`, queried 2026-09-09**: strategy `WF_7_30_NQ_H_CW_H_O_H1_2.34.172` on Darwinex demo **SBDEMO2**, the only one of 123 strategies with both demo trades and backtest runs loaded. Demo (47 trades, +$897.51) diverges from Backtest Deploy (42, +$300.90) and Evaluation (39, +$599.24), and **most of the gap sits in the period where both series trade the same days**, not the disjoint ones. Root cause, confirmed from SQX screenshots: the Data Manager bound data symbol `USATECHIDXUSD_M1_UTC02` (Dukascopy) to instrument `NDX_DARWINEX` — **the instrument spec is Darwinex, the price series is Dukascopy** — producing a **systematic, drifting 22-41 point entry-price offset** across 24 minute-paired trades, worked through a same-signal TP-vs-SL example. Also documents an **independent** modelling gap (AlgoWizard runs "1 minute data tick", not real ticks, so intrabar SL/TP order is assumed), **rules out DST/timezone** as the cause (the offset's sign is never random), and independently confirms the cost model by deriving point value from price arithmetic (exactly 10.0000 on demo, 9.81-12.5 on backtest, cost embedded and measurable). States plainly it **cannot** show demo outperforms the backtest — only 24 paired signals, not enough to establish direction |
...
| Judging whether a backtest is validated against the instrument it will trade | **`MEASURED_Demo_vs_Backtest_Divergence.md`** — a measured case where the instrument specification and the price series were bound to different brokers, producing a systematic, drifting entry-price offset; also `01_SQX_Data.md` § *Tabla de instrumentos* for the doctrine that spec and price series are separate Data Manager bindings |
