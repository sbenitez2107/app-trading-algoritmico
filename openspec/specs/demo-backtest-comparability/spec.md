...
> **Clock divergence is DISCLOSED, never corrected — and the pairing key is what makes that safe.**
>
> The two sides carry different calendars: demo `OpenTime` comes from a Darwinex MT4 statement, while
> the backtest export's Data Manager records timezone **UTC+02 Jerusalem with DST**, whose rules
> differ from the US rules Darwinex follows. At the daylight-saving transition boundaries in late
> March and late October or November the two calendars diverge for a week or two.
>
> **No timezone conversion is performed on either side, and none may be added.** The reason is that
> an hour's shift *moves the minute key*, so a divergence does **not** corrupt an offset — the
> affected trades simply **stop pairing**. The failure signature is therefore a collapse in
> `PairedTradeCount`, which already travels beside every reported figure, rather than a plausible-
> looking wrong number that nothing would flag.
>
> Disclosure is the non-nullable computed `DstTransitionRisk` on each month row, set to
> `TransitionMonth` when the calendar month is 3, 10 or 11. That is a **deliberately over-inclusive
> calendar rule, not a tzdata lookup** — it costs a false positive in months that happen to be clean,
> and buys no dependency and no conversion. Replacing it with a precise timezone lookup, or
> normalizing the two sides to a common zone, would destroy the property above and is forbidden.
>
> Measured evidence that a clock error is not what the current offset shows: all 24 paired trades in
> `.agents/knowledge/imox/MEASURED_Demo_vs_Backtest_Divergence.md` carry a **positive** offset, while
> a timezone error would pair different market moments and produce randomly-signed differences.
